...
/**
 * This is a slightly modified version of play.libs.OAuth2.
 */
public class OAuth2 {
	...
	public OAuth2(String authorizationURL, String accessTokenURL,
	...
	public void retrieveVerificationCode(String callbackURL) {
		throw new Redirect(authorizationURL + "?response_type=code"
				+ "&client_id=" + clientid + "&redirect_uri=" + callbackURL);
	}

	public void retrieveVerificationCode() {
		retrieveVerificationCode(Request.current().getBase()
				+ Request.current().url);
	}
	...
	public Response retrieveAccessToken() {
		return retrieveAccessToken(Request.current().getBase()
				+ Request.current().url);
	}
	...
	public static class Response {
		...
		private Response(String accessToken, Error error,
	...
	public static class Error {
		...
		static Error oauth2(WS.HttpResponse response) {
			if (response.getQueryString().containsKey("error")) {
				...
				return new Error(Type.OAUTH, qs.get("error"),
						qs.get("error_description"));
			} else if (response.getContentType().startsWith("text/javascript")) { // Stupid
																					// Facebook
																					// returns
																					// JSON
																					// with
				...
				return new Error(Type.OAUTH, jsonResponse.getAsJsonPrimitive(
						"type").getAsString(), jsonResponse.getAsJsonPrimitive(
						"message").getAsString());
			} else {
